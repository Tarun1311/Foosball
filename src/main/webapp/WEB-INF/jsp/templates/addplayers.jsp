							...
							<c:forEach items="${listPlayers }" var="employee">
								<tr>
									...
									<td><button class="btn btn-danger"
											onclick="deleteEmp(${employee.id })">Delete</button> <span>
									</span>
										<button class="btn btn-warning"
											onclick="updateEmp(${employee.id },'${employee.employeeId }','${employee.employeeName }')">Update</button></td>
								</tr>
							</c:forEach>
						</table>
					</div>
		  ...
		  xhttp.onreadystatechange = function() {
		    if (this.readyState == 4 && this.status == 200) {
		      ...
		      if(result.error==true){
		    	  document.getElementById("error").innerHTML= "<span>EmpplyeeId already exists in database!</span>";
		      }
		      else{
		    	var listPlay = result.list;//JSON.parse(this.responseText);
